...
def assert_almost_equal_geojson(g1, g2, delta=1e-4, verbose=False):
  ...
  for key in g1.keys():
    if isinstance(g1[key], dict):
      assert_almost_equal_geojson(
          g1[key], g2[key], delta=delta, verbose=verbose
      )
...
class Test5AreaNoticeSimple:
  ...
  def testSimple(self):
    ...
    assert (
        len(an1.get_bits(include_dac_fi=False)) == 10 + 7 + 4 + 5 + 5 + 6 + 18
    )
    assert (
    ...
    assert (
        len(
            an1.get_bits(
                include_bin_hdr=True, mmsi=123456789, include_dac_fi=True
            )
        )
        == 38 + 2 + 16 + 10 + 7 + 4 + 5 + 5 + 6 + 18
    )
...
class TestBitDecoding:
  ...
  def testRectangle(self):
    ...
    rect.add_subarea(
        area_notice.AreaNoticeRectangle(-69.8, 42, 4000, 1000, 0)
    )
    ...
    decoded = geojson.loads(
        geojson.dumps(
            area_notice.AreaNotice(
                nmea_strings=[line for line in rect.get_aivdm()]
            )
        )
    )
    assert_almost_equal_geojson(orig, decoded)
  ...
  def testSector(self):
    ...
    decoded = geojson.loads(
        geojson.dumps(
            area_notice.AreaNotice(
                nmea_strings=[line for line in sec1.get_aivdm()]
            )
        )
    )
    assert_almost_equal_geojson(orig, decoded)
  ...
  def testLine(self):
    ...
    line1.add_subarea(
        area_notice.AreaNoticePolyline([(10, 2400)], -69.8, 42.4)
    )
...
class TestBitDecoding2:
  ...
  def testManySectors(self):
    ...
    notice.add_subarea(
        area_notice.AreaNoticeSector(-69.8, 39.5, 6000, 10, 40)
    )  # 1
    notice.add_subarea(
        area_notice.AreaNoticeSector(-69.8, 39.5, 5000, 40, 80)
    )  # 2
    notice.add_subarea(
  ...
  def testFullText(self):
    ...
    notice.add_subarea(
        area_notice.AreaNoticeCirclePt(-69.5, 42, radius=0)
    )  # 1
...
class TestWhaleNotices:
  ...
  def testNoWhales(self):
    ...
    circle.add_subarea(
        area_notice.AreaNoticeCirclePt(-69.8, 42.0, radius=4260)
    )
