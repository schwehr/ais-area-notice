...
def env_samples():
    ...
    env_dump(
        e,
        "The 3d currents sensor report can not handle west, south, or down?!?",
    )
...
def dump_all(area_notice, kmlfile, byte_align=False):
    ...
    print()
    kmlfile.write(
        area_notice.kml(with_style=True, with_time=True, with_extended_data=True)
    )
    kmlfile.write("\n")
