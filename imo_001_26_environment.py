...
class SensorReport(object):
    ...
    def __eq__(self,other):
        ...
        for key in self.__dict__:
            # TODO(schwehr): Should we skip checking the year and month as they are not really part of the message?
...
class SensorReportWind(SensorReport):
    ...
    def get_bits(self):
        ...
        bits = binary.joinBV(bv_list)
        if len(bits) != SENSOR_REPORT_SIZE:
            raise AisPackingException('bit length'+str(len(bits))+'not equal to'+str(SENSOR_REPORT_SIZE))
        return bits
...
class SensorReportWeather(SensorReport):
    ...
    def __unicode__(self):
        ...
        r.append('\tair_temp={air_temp} air_temp_data_descr={'
                 'air_temp_data_descr} - {air_temp_data_descr_str}'.format(air_temp_data_descr_str=air_temp_data_descr_str, **self.__dict__))
